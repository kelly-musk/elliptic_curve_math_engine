...
use elliptic_curve_math_engine::cli::handler::{handle_generate, handle_derive, handle_info};
...
fn main (){
    ...
    match args.command {
        Commands::Generate { format, show_private } => {
            handle_generate(format, show_private);
        }
        Commands::Derive { private_key, format } => {
